            ...
            async with session.get(API_URL, params=params) as r:
                if r.status != 200:
                    return {
                        "gateway": "Normal",
                        "price": "0",
                        "response": f"HTTP_ERROR_{r.status}"
                    }

                data = await r.json()
                return {
                    "gateway": data.get("Gateway", "Normal"),
...
async def handle_msh(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(raw) < 2:
        await msg.reply_text(
            "⚠️ Uso: <code>/msh lista_de_ccs</code>",
            parse_mode="HTML",
            reply_to_message_id=msg.message_id
        )
        return
    ...
    if len(lines) > MAX_CCS:
        await msg.reply_text(
            f"❌ Máximo permitido: {MAX_CCS} CCs",
            reply_to_message_id=msg.message_id
        )
        return
    ...
    try:
        for card in cards:
            ...
            site_i = sites.index(site) + 1
            proxy_i = proxies.index(proxy) + 1
            ...
            blocks.append(
                ...
                f"<b>Site:</b> {site_i} | <b>Proxy:</b> {proxy_i}\n"
            )
        ...
        final = (
            ...
            + "\n".join(blocks)
            + f"━━━━━━━━━━━━━━━━"
            + f"\n{BOT_TAG} <b>Total:</b> {len(cards)} | <b>Time:</b> {elapsed:.2f}s\n"
...
def register_handlers(application):
    ...
    application.add_handler(
        MessageHandler(filters.Regex(r"^\.msh\b"), handle_msh)
    )